...
const MapboxMap: React.FC<Props> = ({
  ...
  const onFiltersChange = (filters: Filters) => {
    if (type === "main" && map) {
      const a = filters.country
        ? ["==", ["get", "country"], filters.country]
        : null;
  ...
  useEffect(() => {
    ...
    const center = (() => {
      ...
      return [14.1, 48.31] as mapboxgl.LngLatLike;
    })();
  ...
  useEffect(() => {
    ...
    map.on("style.load", () => {
      ...
      const marker = new mapboxgl.Marker({ draggable: true }).setLngLat(
      ...
      if (type === "marker") {
        ...
        marker.on(
          "dragend",
          () => onMarkerPosChange && onMarkerPosChange(marker.getLngLat())
        );
      }
          ...
          >
            ...
            >
              <MapFilters
                ...
                countries={[""].concat(
                  Array.from(new Set([...data!.map((i) => i.country)]))
                )}
                types={[""].concat(
                  Array.from(new Set([...data!.map((i) => i.type)]))
                )}
                capacity={{
                  min: Math.min(
                    ...Array.from(new Set([...data!.map((r) => r.capacity)]))
                  ),
                  max: Math.max(
                    ...Array.from(new Set([...data!.map((r) => r.capacity)]))
                  ),
