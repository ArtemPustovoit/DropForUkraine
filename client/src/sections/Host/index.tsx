...
export const Host = ({ viewer, form }: Props & FormComponentProps) => {
  ...
  useEffect(() => {
    if (location.lng || location.lat) console.log([location.lng, location.lat]);
  }, [location]);
  ...
  return (
    <Content className="host-content">
      <Form layout="vertical" onSubmit={handleHostListing}>
        ...
        <Item label="Your place on map">
          {getFieldDecorator("map", {
            rules: [
              {
                required: false,
                message: "Please provide a position on a map for your place!",
              },
            ],
          })}
          <AddLocation />
